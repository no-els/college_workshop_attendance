...
export const runValidation = (validateFn, value) => {
    const errorMsg = validateFn(value);
    if (errorMsg != null) {
        throw new Error(errorMsg);
    }
};
export const chain = (...validateFns) => (validateFns.reduce((prevFn, fn) => (val) => prevFn(val) || fn(val)));
export const OR = (...validateFns) => (value) => {
    const errorMsgs = [];
...
export const meaningfulType = (value) => {
    if (value === null) {
        return 'null';
    }
...
`.trim().split(/[ \n]/).map((s) => s.trim()));
export const isReserved = (name) => reservedKeywords.has(name);
export const validIdentifier = (jsx) => (name) => {
...
export const nodeAlias = (alias) => (value) => {
    if (value == null || !(value.type in aliases[alias])) {
...
export const arrayOf = (validateFn) => (value) => {
